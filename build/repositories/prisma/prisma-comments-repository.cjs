...
var prisma_comments_repository_exports = {};
__export(prisma_comments_repository_exports, {
  PrismaCommentsRepository: () => PrismaCommentsRepository
});
module.exports = __toCommonJS(prisma_comments_repository_exports);
...
async function get2(request, reply) {
  const getParamsSchema = import_zod9.default.object({
    postId: import_zod9.default.string().uuid()
  });
  const { postId } = getParamsSchema.parse(request.params);
  try {
    const prismapostsRepository = new PrismaPostsRepository();
...
async function getByUserId(request, reply) {
  const getParamsSchema = import_zod10.default.object({
    userId: import_zod10.default.string().uuid()
  });
  const { userId } = getParamsSchema.parse(request.params);
  try {
    const prismapostsRepository = new PrismaPostsRepository();
...
async function create2(request, reply) {
  const createBodySchema = import_zod11.default.object({
    ...
    postId: import_zod11.default.string().optional(),
    commentId: import_zod11.default.string().optional()
  }).refine(
...
async function deleteLike(request, reply) {
  const getParamsSchema = import_zod12.default.object({
    likeId: import_zod12.default.string().uuid()
  });
...
async function createComment(request, reply) {
  const schema = import_zod13.default.object({
    ...
    created_at: import_zod13.default.string().transform((str) => new Date(str))
  });
...
async function updateComment(request, reply) {
  const { commentId } = request.params;
  const userId = request.user.sub;
  const schema = import_zod14.default.object({
