...
async function get(request, reply) {
  const getParamsSchema = import_zod3.default.object({
...
var AuthenticateUseCase = class {
  ...
  async execute({ email, password }) {
    ...
    const doesPasswordCompare = await (0, import_bcryptjs2.compare)(password, user.password);
...
async function authenticate(request, reply) {
  const authenticateBodySchema = import_zod4.default.object({
    email: import_zod4.default.string().email(),
    password: import_zod4.default.string().min(6)
  });
  const {
    email,
    password
...
async function create(request, reply) {
  const createBodySchema = import_zod5.default.object({
    title: import_zod5.default.string(),
    created_at: import_zod5.default.string().transform((str) => new Date(str)),
...
async function deletePost(request, reply) {
  const getParamsSchema = import_zod6.default.object({
    postId: import_zod6.default.string().uuid()
  });
...
async function update2(request, reply) {
  const updateParamsSchema = import_zod7.default.object({
    postId: import_zod7.default.string().uuid()
  });
  const updateBodySchema = import_zod7.default.object({
    title: import_zod7.default.string().optional(),
    content: import_zod7.default.string().optional()
  });
  const userId = request.user.sub;
...
async function get2(request, reply) {
  const getParamsSchema = import_zod8.default.object({
...
var UpdateUserUseCase = class {
  ...
  async execute({ userId, data }) {
    ...
    if (data.password) {
      const isSamePassword = await (0, import_bcryptjs3.compare)(data.password, user.password);
      if (isSamePassword) {
        throw new Error("As senhas devem ser diferentes!");
      }
      data.password = await (0, import_bcryptjs3.hash)(data.password, 6);
    }
...
async function update(request, reply) {
  const updateParamsSchema = import_zod15.default.object({
    userId: import_zod15.default.string().uuid()
  });
  const updateBodySchema = import_zod15.default.object({
    name: import_zod15.default.string().optional(),
    email: import_zod15.default.string().optional(),
    photo: import_zod15.default.string().optional(),
    password: import_zod15.default.string().optional()
  });
