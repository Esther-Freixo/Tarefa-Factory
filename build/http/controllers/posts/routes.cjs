...
function likesRoutes(app2) {
  app2.post("/like", { onRequest: [verifyJWT] }, create);
...
async function create2(request, reply) {
  const createBodySchema = import_zod11.default.object({
    title: import_zod11.default.string(),
    created_at: import_zod11.default.string().transform((str) => new Date(str)),
...
async function update2(request, reply) {
  const updateParamsSchema = import_zod13.default.object({
    postId: import_zod13.default.string().uuid()
  });
  const updateBodySchema = import_zod13.default.object({
    title: import_zod13.default.string().optional(),
    content: import_zod13.default.string().optional()
  });
  const userId = request.user.sub;
...
async function get2(request, reply) {
  const getParamsSchema = import_zod14.default.object({
    postId: import_zod14.default.string().uuid()
  });
  const { postId } = getParamsSchema.parse(request.params);
  try {
    const prismapostsRepository = new PrismaPostsRepository();
...
function postsRoutes(app2) {
  app2.post("/posts", { onRequest: [verifyJWT] }, create2);
  app2.get("/posts", getAll3);
